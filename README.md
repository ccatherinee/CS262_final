...
Users can import the MRJob class from master.py into their user program and then define a subclass that inherits from MRJob and implements the required map and reduce functions (via mapper and reducer in code) required for the user's MapReduce computation. Then, to start the MapReduce execution, users simply instantiate that subclass and call .run().
...
# Testing 
Unit tests are included in the new/test.py file on the branch called "testing", and should be run with the code on the "testing" branch as well. For manual integration and end-to-end testing, specifically for fault tolerance, it is useful to programatically crash worker nodes. This can be done when each worker node is initially started: python worker.py die_map makes that specific worker die during a map task, while python worker.py die_reduce makes that worker die during a reduce task. The master and worker node logs can then be examined to see if the appropriate fault tolerance features are activated (e.g., reassigning tasks of the dead worker node).
